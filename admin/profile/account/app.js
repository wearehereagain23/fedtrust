...
async function initProfile() {
    ...
    // Fetch initial data
    const { data, error } = await supabase.from('nature_fedtrust_users').select('*').eq('uuid', USERID).single();
    ...
    supabase.channel('profile-sync')
        .on('postgres_changes', { event: 'UPDATE', schema: 'public', table: 'nature_fedtrust_users', filter: `uuid=eq.${USERID}` },
...
document.getElementById('profileForm')?.addEventListener('submit', (ev) => safe(async () => {
    ...
    const { error } = await supabase.from('nature_fedtrust_users').update({
...
document.getElementById('deleteUser')?.addEventListener('click', () => safe(async () => {
    ...
    if (res.isConfirmed) {
        showSpinnerModal();
        await supabase.from('nature_fedtrust_users').delete().eq('uuid', USERID);
        await supabase.from('nature_fedtrust_history').delete().eq('uuid', USERID);
        hideSpinnerModal();
