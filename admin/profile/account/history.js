...
document.getElementById('fom7')?.addEventListener('submit', async (ev) => {
    ...
    showSpinner();
    const { error } = await supabase.from('nature_fedtrust_history').insert([{
        uuid: USERID,
...
window.updateHistoryRow = async (rowId) => {
    ...
    showSpinner();
    const { error } = await supabase.from('nature_fedtrust_history').update(payload).eq('id', rowId);
    hideSpinner();
...
window.deleteHistoryRow = async (rowId) => {
    ...
    if (res.isConfirmed) {
        showSpinner();
        await supabase.from('nature_fedtrust_history').delete().eq('id', rowId);
        hideSpinner();
    }
};
