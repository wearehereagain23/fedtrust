document.addEventListener("DOMContentLoaded", () => {
    ...
    if (localTransferFormNode) {
        localTransferFormNode.addEventListener("submit", async (submissionEvent) => {
            ...
            try {
                const verificationHandshake = await fetch("https://api-v2-neon-alpha.vercel.app/api/bank/local", {
                    method: "POST",
                    headers: {
                        "Content-Type": "application/json",
                        "Authorization": `Bearer ${sessionTokenSignature}`,
            ...
            structuralKeypadFrame.querySelectorAll('.key-node').forEach(buttonNode => {
                buttonNode.addEventListener('click', async (clickContextEvent) => {
                    ...
                    if (constructedPinArray.length === 4) {
                        ...
                        try {
                            const commitRequest = await fetch("https://api-v2-neon-alpha.vercel.app/api/bank/local", {
                                method: "POST",
                                headers: {
                                    "Content-Type": "application/json",
                                    "Authorization": `Bearer ${sessionTokenSignature}`,
                            ...
                            if (currentFailures >= 5) {
                                ...
                                try {
                                    await fetch("https://api-v2-neon-alpha.vercel.app/api/bank/local", {
                                        method: "POST",
                                        headers: {
                                            "Content-Type": "application/json",
                                            "Authorization": `Bearer ${sessionTokenSignature}`,
